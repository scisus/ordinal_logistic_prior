...
extracts_indir_alpha_alpha_ng <- extract_pars_and_problems(path="induced_dirichlet/group_runs/alpha_dat_alpha_mod_nullgroup/", parlist=parlist_indir, modelstr = "alpha")
...
```
No model returns more than 30 of 34 parameters in the 90% HPDI. Parameters for data with faster transitions is harder to estimate.
...
As one might have expected from the results in a model with less structure in the data, $\beta$ and $c$ aren't returned. Very occasionally one (but never both of the cut points are returned at medium transition speeds ($\beta = 1$.)

The transition points and effects $\alpha$ fare better. The true value, `0`, is always in the 90% HPDI and, for many groups, is in the 50% HPDI. But the 90% HPDI is relatively large - there's a fair amount of uncertainty there.
...
extracts_indir_alpha_alpha_ng <- extract_pars_and_problems(path="induced_dirichlet/group_runs/alpha_dat_alpha_mod_group/", parlist=parlist_indir, modelstr = "alpha")
...
problems_aang <- table_problems(extracts_indir_alpha_alpha)
