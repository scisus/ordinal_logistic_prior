...
```{r readModels, warning=FALSE}

# pull in parameters and info on divergences, etc from saved stanfit objects

extracts_gam <- extract_pars_and_problems(path="gamma/covar_runs/")
```
### Model check
...
knitr::kable(problems_gam, caption = "Models with divergences, rhats > 1, or neff_ratios < 0.1")

```
18/27 models had problems with Rhat or neff_ratio in 1-5 of the 30 model runs. No divergences ever though. 
...
```{r GammaHDPI, message=FALSE, warning=FALSE}
...
ggplot(prop_recaptured50, aes(x=gammaid, y=as.factor(beta_rate), fill=mean_captured)) + 
...
ggplot(prop_recaptured90, aes(x=gammaid, y=as.factor(beta_rate), fill=mean_captured)) + 
  geom_tile(colour="white") +
  geom_text(aes(label=modelid)) +
  facet_wrap("transition", scales="free") +
...
perform50_summary <- perform50 %>%
  group_by(modelid, gammaid, beta_rate, beta, param) %>%
...
ggplot(perform50_summary, aes(x=param, y=as.factor(modelid), fill=prop_inint)) + 
...
map(params_gam[sample(1:30, 5)], parplot) # pick 5 models at random to plot
...
ggplot(prop_recaptured_by_param$ninety, aes(x=param, y=as.factor(modelid), fill=prop_inint)) + 
  geom_tile(color="black") +
  scale_fill_viridis_c()  +
  geom_text(aes(label=paste(anchor, beta_rate, sep="\n")), size=2.5) +
  facet_wrap("transition", scales="free") +
